...
网络视频默认请求视频和音频并合并为 MKV，完成后使用同目录的 ffprobe 检查两种流；仅有视频的旧文件不会误报下载完成。下载日志可展开/收起。网络下载支持按行输入多个链接、画质上限（至 4K）、音频/封面下载，以及字幕、播放列表范围、限速、重试、代理、命名模板和自定义参数。高级参数需点击“应用参数”后生效。
